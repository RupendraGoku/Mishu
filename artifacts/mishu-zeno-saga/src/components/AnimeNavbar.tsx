    ...
    <>
      ...
      >
        <div className="container mx-auto px-4 md:px-8 flex justify-between items-center">
          ...
          <nav className="hidden md:flex items-center gap-6">
            {links.map(link => (
              <a 
                ...
                className="font-display uppercase tracking-wider text-soft-white/80 hover:text-electric-cyan hover:text-glow-blue transition-all text-sm lg:text-base"
              >
                {link.name}
              </a>
            ))}
          ...
          {/* Mobile Toggle */}
          <button 
            className="md:hidden text-energy-blue"
            onClick={() => {
              playSfx('hover');
              setMobileOpen(true);
            }}
      ...
      <AnimatePresence>
        {mobileOpen && (
          <motion.div 
            ...
            className="fixed inset-0 z-50 bg-space-navy/95 backdrop-blur-md flex flex-col border-l-4 border-energy-blue"
          >
            ...
            <div className="flex flex-col gap-6 px-12 py-8 mt-auto mb-auto relative z-10">
              ...
              {links.map((link, i) => (
                <motion.a
                  ...
                  href={link.href}
                  onClick={(e) => { e.preventDefault(); handleNavClick(link.href); }}
                  className="font-display text-3xl uppercase tracking-widest text-soft-white hover:text-electric-cyan hover:pl-4 transition-all flex items-center gap-4"
                >
                  <span className="text-energy-blue text-sm">[{String(i+1).padStart(2, '0')}]</span>
                  {link.name}
                </motion.a>
              ))}
            </div>
